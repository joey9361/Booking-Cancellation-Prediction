...
from config.settings import MOTEL_DIRECT_CHANNEL, BOOKING_KEY
from sklearn.preprocessing import OneHotEncoder
from exceptions import CustomException
import sys
from artifacts.artifacts import save_preprocessing_artifacts
...
def fit_room_code(train_df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Get all distinct (property_name, unit_code) pairs as rows
    units_table = (
...
def booking_level_preprocessing(df: pd.DataFrame, is_offline: bool) -> pd.DataFrame:
    ...
    bookings = df.groupby(BOOKING_KEY, as_index=False).aggregate(
...
def run_offline_preprocessing(
    ...
    """
    Run offline preprocessing on a dataframe, transforms data, creates preprocessing 
    artifacts and saves them to disk, returning X_Train, Y_Train, full_val, full_test, 
    the model compatible dfs/Series.
    """
