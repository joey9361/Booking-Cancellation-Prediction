...
from config.settings import (
    ...
    MODEL_PATH, MIN_ACCEPTED_PRECISION, ARTIFACTS_PATH
    )
from joblib import dump
from artifacts.artifacts import save_prediction_artifacts, save_threshold_artifacts, load_preprocessing_artifacts   
...
def main(
    ...
    # Record TRAIN_INFERENCE_TIME_CUTOFF
    ...
    logging.info(f"""
                Evaluation results: Weighted F1 Score: {f1Scores}, Recall: {recallScore}, 
                Precision: {precisionScore}, Confusion Matrix: {confusionMatrix}
                """)
